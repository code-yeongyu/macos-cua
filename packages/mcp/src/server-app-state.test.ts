...
beforeEach(() => {
	...
	mockedComputer.getAppState.mockResolvedValue(stateWithCursor());
});
...
describe("MCP get_app_state #given cursor metadata #when screenshot is JPEG #then the returned image includes the cursor", () => {
	it("#given a cursor inside the app window #when get_app_state returns content #then image is converted to PNG", async () => {
		...
		expect(image.data).not.toBe(stateWithCursor().screenshotBase64);
		const bytes = Buffer.from(image.data, "base64");
		expect(decodeImageSize(bytes)).toEqual({ width: 160, height: 120 });
		expect(await pixelAt(bytes, 20, 20)).toEqual([255, 59, 48, 255]);
	});
});
...
function stateWithCursor(): AppState {
	const windowBounds = { x: 10, y: 20, width: 160, height: 120 };
	const cursor = { x: 30, y: 40 };
	return {
		app: "Finder",
...
function decodeImageSize(data: Buffer): { readonly width: number; readonly height: number } {
	const image = new Image();
	image.src = data;
	return { width: image.width, height: image.height };
}

async function pixelAt(data: Buffer, x: number, y: number): Promise<readonly number[]> {
