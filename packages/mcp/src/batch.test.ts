...
async function createClient(): Promise<{ readonly client: Client; readonly close: () => Promise<void> }> {
	...
	const client = new Client({ name: "macos-cua-batch-test", version: "0.1.0" });
...
function batchText(result: { readonly content?: unknown }): string {
	if (!Array.isArray(result.content)) {
		throw new Error("batch result content must be an array");
	}
	const text = result.content.find((content): content is { readonly type: "text"; readonly text: string } => {
		return (
...
describe("MCP batch tool #given get_app_state then click #when run through the client #then coordinates re-anchor", () => {
	it("#given in-batch app state #when click follows #then it maps through the latest capture frame", async () => {
		...
		expect(JSON.parse(batchText(result))).toMatchObject({
			ok: true,
			code: "BATCH_COMPLETED",
			steps: [
				{ index: 0, action: "get_app_state", ok: true },
				{ index: 1, action: "click", ok: true },
			],
		});
	});
