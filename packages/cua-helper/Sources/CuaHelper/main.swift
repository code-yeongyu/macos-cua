...
private let defaultDragSteps = 20
private let maximumDragSteps = 200
...
private func toPoint(_ request: HelperRequest) throws -> CGPoint {
	guard let x = request.toX else { throw HelperFailure.missing("toX") }
	guard let y = request.toY else { throw HelperFailure.missing("toY") }
	return CGPoint(x: x, y: y)
}

private func button(_ request: HelperRequest, fallback: MouseButton) -> MouseButton {
